...
chosen_proportion <- 0.3
```

```{r step 2 chr19 subsample per gene expression quantile, eval = F}
# load required files and set directories:
...
### output dir:
dir_out_files <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/chr19_bw_tss_subsample/"
...
resize_bw <- function(w_in_kb, f, tss){
  ...
  sum(width(tss_window_reduce))
  bw_tss_window_reduce <- import(paste(dir_raw_files, f, sep = "/"), which = tss_window_reduce, as = "RleList")
  bw_out <- file.path(dir_out_files, 
                      paste0(tools::file_path_sans_ext(f), "_chr19tss_", chosen_proportion, "ofGenes_", w_in_kb, "kb_window", ".bw"))
  export.bw(object = bw_tss_window_reduce, bw_out)
}

## perform function on all files in input directory. 
for(file in bw_files){
resize_bw(20, file, tss_chr19_sample)
}

## save sliced quantification and TSSs
save(quantification_chr19_sample, tss_chr19_sample, file = "inst/extdata/week3.Rdata"
```
