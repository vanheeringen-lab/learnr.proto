...
### chr19 GRanges of transcript-centered genomic features  
...
### chr19 df of gene symbols and entrezIDs  
Not used anymore in practical (2021 version)  
...
### chr19 df of ENCODE cis-regulatory elements  
Not used anymore in practical (2021 version)  
...
#### chr19 Alternative gene ID  mapping with biomaRt:  
...
### Select bigwig files for chr19 limited to regions TSS +/- 20kb   
**2020-01-17**  
...
```
<br>
### chr21 - Test bigwig size of chr21 TSS windows - not used
...
## chr21 peaks and quantification for week 3 - not used  
...
#### store chr21 granges and quantification  
```{r save prepare monocyt data, echo = T, eval = F}
...
3. Random select subsample of genes from chr19 based on quantification quartiles.  

```{r chr19 subsample per gene expression quantile, eval = F}
all_results <- read.table("/home/ctoenhake/learnr.proto/inst/extdata/week3/quant/C000S5B1.gene_quantification.rsem_grape2_crg.GRCh38.20150622.results", header =T)
# remove version from ensemble gene id 
all_results$gene_id <- sapply(all_results$gene_id,  sub, pattern = "\\.\\d+$", replacement = "")
# map entrez id to ensemble id
all_results$entrezgene_id = mapIds(org.Hs.eg.db,
...
# make 4 quantiles and subsample per quantile  
quantification_chr19_slice <- quantification_chr19 %>% 
  mutate(expression_group = ntile(FPKM, 4)) %>% 
  group_by(expression_group) %>% 
  slice_sample(prop=0.3, replace = F)

# check number of genes per quantile and in expression plot
table(quantification_chr19_slice$expression_group)
ggplot(quantification_chr19, aes(x = as.factor(expression_group), y = log10(FPKM+1)))+
  geom_boxplot()+
  geom_jitter()
ggplot(quantification_chr19_slice, aes(x = as.factor(expression_group), y = log10(FPKM+1)))+
  geom_boxplot()+
  geom_jitter()

```

```{r subsample bigiwg file for these TSS windows, eval = F}
library(TxDb.Hsapiens.UCSC.hg38.knownGene)
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene 
seqlevels(txdb) <- seqlevels0(txdb)
seqlevels(txdb) <- "chr19"
tss_chr19 <- unique(promoters(genes(txdb), upstream = 2000, downstream = 0 )) 
tss_chr19 <- resize(tss_chr19, width = 1, fix = "end")
tss_chr19_slice <- tss_chr19 %>% filter(gene_id %in% quantification_chr19_slice$entrezgene_id )  

dir_raw_files <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/chr19_bw/"

chr19_subsample_tss <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/chr19_bw_tss_subsample/"
  
bw_files <- dir(path = dir_raw_files, pattern = "*.bw")
bw_names <- sapply(bw_files,  sub, pattern = "\\$.bw", replacement = "")
resize_bw <- function(w_in_kb, f, tss){
  print(deparse(substitute(f)))
  w_in_bp <- w_in_kb*1000+4
  tss_window <- resize(tss, width = w_in_bp, fix = "center")
  tss_window_reduce <- reduce(tss_window)
  sum(width(tss_window_reduce))
  bw_h3k4me3_tss_chr21_window <- import(paste(dir_raw_files, f, sep = "/"), which = tss_window_reduce, as = "RleList")
  bw_out <- file.path(chr19_subsample_tss, 
                      paste(tools::file_path_sans_ext(f), "chr19_TSSsubsample", w_in_kb, "kb.bw", sep = "_"))
  export.bw(object = bw_h3k4me3_tss_chr21_window, bw_out)
}
for(file in bw_files){
resize_bw(20, file, tss_chr19_slice)
}
