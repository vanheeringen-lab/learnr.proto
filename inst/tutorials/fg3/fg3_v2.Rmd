...
tss_chr19$names <- names(tss_chr19)
...
<br>
You will start by identifying promoters overlapping a H3K4me3 peak and plot the ChIP-seq signal in these regions. This signal can be obtained from .bam files or from .bigwig files. 
<br>
Let's get started! 
...
First, you will assign H3K4me3 ChIP-seq peaks to their closest TSS. You will use the r package [genomation](https://github.com/BIMSBbioinfo/genomation) to do this. We have installed and loaded this package for you with the following code:  
```{r installing genomation, echo = T, eval = F}
BiocManager::install("genomation")
library(genomation)
```

From this package you will use the function:  
`distance2NearestFeature(g.idh = [GRanges object withe peaks], tss = [GRanges object with TSSs])` 

This function outputs a data.frame reporting for each range in the `g.idh` object, its nearest TSS. It also gives you the index and name of the corresponding peak and tss, respectively. And the distance between them.   
...
```{r q1_distancetss, exercise = TRUE, exercise.lines = 5, exercise.eval = F, error = T}
# Identify the nearest TSS for each H3K4me3 peak  
nearest_tss_to_h3k4me3 <- __(g.idh = __, tss = __)

# Show the resulting object 

```

```{r q1_distancetss-hint-1,  eval = F}
# use the function
distance2NearestFeature()
```

```{r q1_distancetss-hint-2,  eval = F}
# within distance2NearestFeature define g.idh as:
g.idh = monocytes_h3k4me3
```

```{r q1_distancetss-hint-3,  eval = F}
# and tss as:
tss = tss_chr19
```

```{r q1_distancetss-hint-4,  eval = F}
# finish with
head(nearest_tss_to_h3k4me3)
```

```{r q1_distancetss-check, eval = F}
grade_result(
  pass_if(~identical(.result, head(distance2NearestFeature(monocytes_h3k4me3, tss_chr19))))
)
```

The `nearest_tss_to_h3k4me3` reports for each H3K4me3 peak a nearest TSS. In total `r length(unique(distance2NearestFeature(monocytes_h3k4me3, tss_chr19)$feature.name` different TSSs are reported, meaning that some peaks are assigned to the same TSS.    

### 3.2.2 Visualize distances  
How far are the H3K4me3 peaks located from the nearest TSS?   
...
> **Exercise 2a:** Print a summary of the distances between H3K4me3 peaks and their nearest TSS using the function:  
`summary(object = [object for which you want a summary, can be vector, matrix or data.frame])`.  
Look at the output from exercise 1b to identify the variable that stores the distance.   

```{r prepare_q2a, echo = F}
nearest_tss_to_h3k4me3 <- distance2NearestFeature(monocytes_h3k4me3, tss_chr19)
```

```{r q2a_summary,  exercise = TRUE, exercise.lines = 2, exercise.eval = F, error = T, exercise.setup = "prepare_q2a"}
# print a summary of the distances

```

<div id="q2a_summary-hint">
We want a summary of `nearest_tss_to_h3k4me3$dist.to.feature`.  
</div>

```{r q2a_summary-check}
grade_result(
  pass_if(~identical(.result,summary(nearest_tss_to_h3k4me3$dist.to.feature)))
)
```

> **Exercise 2b:** Also plot the distances from `nearest_tss_to_h3k4me3` using ggplot() and geom_boxplot().     

```{r q2b_plotdistances, exercise = TRUE, exercise.lines = 4, exercise.eval = F, error = T, exercise.setup = "prepare_q2a"}
# plot the distances as histogram with ggplot
ggplot(___)+
  geom_boxplot(aes(y = ___))+
  scale_x_continuous(breaks = NULL) # we remove the breaks from the x axis because we have only 1 category 
```

```{r q2b_plotdistances-solution, eval =F}
# plot the distances as histogram with ggplot
ggplot(nearest_tss_to_h3k4me3)+
  geom_boxplot(aes(y = dist.to.feature))+
...
Observe that ~50% of the datapoints fall within 10kb around the TSS and the extreme outliers we are having; peaks are assigned to TSSs that are over 100kb away! 

### 3.2.3 Filter on distances  
Most "distances" are within 10kb and we will filter for matches within that distance. 

### Exercise 3  
Use dplyr::filter filter `nearest_tss_to_h3k4me3` for matches that are within 10kb distance.    

>#### **Refresher**: dplyr     
<details><summary>Common dplyr-functionswith examples</summary>   
>
Function | Purpose | Example     
:--|:----       
`select()` | subset variables (=columns) | select TSS column: `nearest_tss_to_h3k4me3 %>% select(feature.name)`  
`group_by()` | group data into rows with the same value for the specified variable. | group by TSS: `nearest_tss_to_h3k4me3 %>% select(feature.name)`   
`filter()` | subset observations (= rows) | filter for TSSs on the "-" strand: `nearest_tss_to_h3k4me3 %>% filter(feature.strand == "-")`  
`summarize()` | Summarise variables, often per group | print the mean distance grouped by strand: `nearest_tss_to_h3k4me3 %>% group_by(feature.strand) %>% summarize(mean = mean(dist.to.feature))`  
`mutate()` | Add a new variable | Add column with absolute distances: `nearest_tss_to_h3k4me3 %>% mutate(abs_distance = abs(dist.to.feature))`
>
See also the [dplyr cheatsheet](https://rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf)  
>
Within filter you can use the logical operators 
...
```{r q3_filter, exercise = TRUE, exercise.eval = F, error = T, exercise.setup = "prepare_q2a"}
# filter nearest_tss_to_h3k4me3 for distances <= 10000
nearest_tss_to_h3k4me3 <- nearest_tss_to_h3k4me3 %>% filter(dist.to.feature <= 10000 & dist.to.feature >= -10000)
```

```{r q3_filter-hint-1, eval = F}
# use as condition  
metadata$distance <= 10000
```

```{r q16a_closegenes-solution, eval = F}
tss_near_h3k4me3 <- tss_near_h3k4me3[metadata$distance <= 10000,]
```
...
```{r prepare_q4, echo = F}
tss_near_h3k4me3 <- distanceToNearest(monocytes_h3k4me3, tss_chr19)
metadata <- as.data.frame(mcols(tss_near_h3k4me3))
tss_near_h3k4me3 <- tss_near_h3k4me3[metadata$distance <= 10000,]
```
...
Currently, the window of the TSS intervals in `tss_chr19` is 1bp while we want to plot the signal *around* the TSS. We can use the function `resize(x = [GRanges object], width = [desired width], fix = ["start", "end" or "center"])` to resize these 1bp intervals to the size that we are interested. As we want to have a window *around* the TSS we will set the `fix =` parameter to "center".    
<br>
...
```{r example_densityplot, exercise = TRUE, exercise.setup = "prepare_exampledensity"}
# plot density plot
