...
suppressPackageStartupMessages({
  ...
  rdata ="/home/ctoenhake/learnr.proto/inst/extdata/week3_v2.Rdata" 
  load(rdata)
  rm(rdata)  
  ...
  bw_h3k4me3_60genes <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/bw_chr19_tss_sample/C000S5H2.ERX547984.H3K4me3.bwa.GRCh38.20150528.chr19_chr19tss_60Genes_10kb.bw"
  bw_h3k4me1_60genes <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/bw_chr19_tss_sample/C000S5H2.ERX547981.H3K4me1.bwa.GRCh38.20150528.chr19_chr19tss_60Genes_10kb.bw"
  bw_h3k27me3_60genes <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/bw_chr19_tss_sample/C000S5H2.ERX547983.H3K27me3.bwa.GRCh38.20150528.chr19_chr19tss_60Genes_10kb.bw"
  bw_h3k27ac_60genes <- "/home/ctoenhake/learnr.proto/inst/extdata/week3/bw_chr19_tss_sample/C000S5H2.ERX547980.H3K27ac.bwa.GRCh38.20150528.chr19_chr19tss_60Genes_10kb.bw"
...
<br>
We limited the data stored in the bigWig files to a **10kb window around the TSSs of 60 genes on chromosome 19 (genes were randomly selected)**. This was done to reduce the size of these files (bigWig and BAM files are huge!) and speed up the tutorial a bit. By taking a *random* selection, the relationships present in the genome-wide dataset should till hold in the sampled dataset. However, because of this decision you can't make plots of windows wider than 10kb. Also, even with this downsampling, the calculations can take 10-20sec, so- more time for coffee ;-)!  
...
>
> * The object `bw_h3k4me3_60genes` directs to the H3K4me3 ChIP-seq bigWig file.  
...
grade_result(
  ...
  correct = "The signal you get with only  60 genes in the dataset is a somewhat noisy. Open the dropdown box of exercise 3c to view the same plot but generated using all TSSs on chromosome 19. Use that example plot to answer the multiple choice questions.")
```
...
grade_result(pass_if(~identical(.result,plotMeta(mat = ScoreMatrix(target = bw_h3k27ac_60genes,
             ...
             correct = "Looks a bit bumpy doesn't it? For exercise 4b we therefore prepared the plot in advance.")
```
...
<!-- scores_h3k27me3_tss <- ScoreMatrix(target = bw_h3k27me3_60genes, windows = tss_window, type = 'bigWig', strand.aware =TRUE)   -->
...
grade_result(pass_if(~identical(.result,
                                plotMeta(mat = new("ScoreMatrixList", list(h3k4me3=scores_h3k4me3_tss, 
                                                                           ...
                                                                           h3k27me3=ScoreMatrix(target = bw_h3k27me3_60genes, windows = tss_window, type = 'bigWig', strand.aware =TRUE))), 
                                         xcoords = c(-5000, 5000), 
                                         overlay = TRUE, 
                                         profile.names = c("H3K4me3", "H3K27ac", "H3K27me3"))
                                )),
...
question("What can you conclude from the above plot?",
         ...
         answer("There are no TSSs with both H3K4me3 and H3K27me3", message = "Although the average H3K27me3 signal is low due to that hsitone mark not showing high enrichments and that likely only a few TSSs in this dataset have this mark, that does not mean that H3K27me3 is not found at promoters that have H3K4me3."),
...
```
<br>
By running `ntile()` within `mutate()` you can define the new variable. For example, below we categorize TSS into quartiles based on the corresponding gene expression stored in the metadata of `tss_chr19_60genes`:   
```{r ntile_example2, echo=TRUE, eval=FALSE}
...
```
<br>
![](images/q7e_scatterplot_average_h3k4me3_tss_5kbwindow_vs_fpkm_incl_default_smoothline.png){width=75%}\   
<br>  
...
```
<br>
![](images/q7e_scatterplot_average_h3k4me3_tss_5kbwindow_vs_fpkm_incl_lm_fitline.png){width=75%}\   
<br>
...
```{r prepare_q8e}
tss_window_60genes <- resize(x= tss_chr19_60genes, width = 5001, fix = "center")
average_h3k27me3_tss <- ScoreMatrixBin(target = bw_h3k27me3,windows = tss_window_60genes,bin.num = 1,type = 'bigWig',strand.aware =TRUE)
average_h3k27me3_tss_matrix <- as(average_h3k27me3_tss, "matrix")
average_h3k27me3_tss_df <- as.data.frame(average_h3k27me3_tss_matrix)
average_h3k27me3_tss_df$gene_id <- tss_chr19_60genes$gene_id
average_h3k27me3_tss_df$FPKM <- tss_chr19_60genes$FPKM
...
<details><summary>Click here for the heatmap of H3K4me3 around all TSSs of chromosome 19.</summary>  
<br>
...
> * `tss_window_60genes` from Exercise 11 is available to you.  
...
kmeans_function <- function(x) kmeans(x, centers = 3)$cluster
# Warning, the next hint is the solution!
```
