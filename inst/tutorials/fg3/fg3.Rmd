...
Instead of looking for overlap between H3K4me3 peaks and promoters of a defined width, this time you will identify the nearest TSS. For this, you will use the r package [genomation](https://github.com/BIMSBbioinfo/genomation), which we used briefly in fg2 as well. This package contains a number of functions to help you analyze epigenomics data. (Side note: there are a number of other packages to analyze ChIP-seq data we well. Incl. ChIPpeakAnno and ChIPseeker.)   
<br>
We have installed and loaded this package for you with the following code:  
```{r install_genomation, echo = T, eval = F}
...
```{r q2a_summary, exercise = TRUE, exercise.eval = F, error = T, exercise.setup = "prepare_q2a"}
...
```{r q2b_plotdistances, exercise = TRUE, exercise.eval = F, error = T, exercise.setup = "prepare_q2a"}
...
question("Which of the following statments are correct?",
         ...
         answer("A binwidth of 10kb, givs us the nicest histogram with sufficient detail but not too peaky.", correct=T),
         allow_retry=T
         )
```
...
The TSS with a 'nearby H3K4me3' peak can be obtained from `dist_h3k4me3_to_tss$feature.name`. This holds the Entrez gene ID which we can use to filter our `tss_chr19` object with.  
...
<br>
The window of the TSS intervals in `tss_chr19` is currently 1bp. But we want to plot the signal *around* the TSS!  
<br>
...
```{r h3k4me3_scores, exercise=TRUE, exercise.eval=F, error=T, exercise.setup="prepare_q2a"}
# define the windows:  
window <- 20000
...
```{r example_densityplot, exercise = TRUE, exercise.eval=F, error=T, exercise.setup = "h3k4me3_scores"}
# plot density plot
...
quiz(caption = "", 
     question("Is H3K4me3 signal equally strong before and after the TSS or is there a difference?",
              ...
              allow_retry = T),
     question("What would the plot show, in case you would have set 'strand.aware = FALSE' in the 'ScoreMatrix()' function? You can adjust the code and try it out.",
...
```{r h3k37me3_signal_tss, exercise=TRUE, exercise.eval=F, error=T, exercise.lines=9, exercise.setup="h3k4me3_scores"}
# 1. Path to bigwig file 
bw_h3k27me3 <- system.file("extdata", "week3",  "C000S5H2.ERX547983.H3K27me3.bwa.GRCh38.20150528.chr19.bw", package = "learnr.proto")
...
scores_h3k27me3_tss <- ScoreMatrix(target = bw_h3k27me3, windows = tss_window_with_h3k27me3, type = 'bigWig', strand.aware = TRUE)  
...
```{r prepare_overlay_density, echo=F}
dist_h3k4me3_to_tss <- distance2NearestFeature(monocytes_h3k4me3, tss_chr19)
window <- 10000
tss_window <- resize(tss_chr19, width = window, fix = "center")
tss_window_with_h3k4me3 <- tss_window %>% filter(name %in% dist_h3k4me3_to_tss$feature.name)
...
```{r overlay_density, exercise=TRUE, exercise.eval=F, error=T, exercise.setup="prepare_overlay_density"}
...
<br>
As the name suggests, this function does not calculate the signal for each bp in the given windows, but for each bin. If we set the `bin.num` parameter to `1`, it will output the average signal over the whole window. Note that this function is also useful when working with windows of varying size like complete genes.  

> **Exercise 6**: Calcualte the average H3K4me3 signal in a window of +/- 5kb around the TSS. Do this for *all* TSSs, irrespective of their overlap of distance to a H3K4me3 peak.   
...
window <- 10000
tss_window <- resize(tss_chr19, width = window, fix = "center")
scores_h3k4me3_tss_all <- ScoreMatrixBin(target = bw_h3k4me3, windows = tss_window, type = 'bigWig', bin.num = 1, strand.aware = TRUE)  # gives the average score over the complete window. And not the sum of 1bp bin windows. 
scores_as_matrix <- as(scores_h3k4me3_tss_all, "matrix")
scores_as_df <- as.data.frame(scores_h3k4me3_tss_all)
scores_as_df$gene_id <- tss_window$gene_id
head(scores_as_df) 

scores_and_quant <- merge(scores_as_df, quantification_chr19, by.x = "gene_id", by.y = "entrezgene_id")
plot(log10(scores_and_quant$FPKM), scores_and_quant$V1)
...
<!-- ```{r example_densityplot, exercise = TRUE, exercise.eval=F, error=T, exercise.setup = "example_scores"} -->
<!-- # plot density plot -->
<!-- plotMeta(scores_h3k4me3_tss, xcoord = c(-5000, 5000)) -->
