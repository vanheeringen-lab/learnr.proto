...
suppressPackageStartupMessages({
  ...
  # add FPKM to TSS windows such that these intervals can be arranged by FPKM
  ...
  bw_h3k36me3 <- system.file("extdata", "week3", "bw_chr19_tss_sample", "C000S5H2.ERX547979.H3K36me3.bwa.GRCh38.20150528.chr19_chr19tss_0.4ofGenes_15kb.bw", package = "learnr.proto")
})
```
...
6. Summarize ChIP-seq signal in a heatmap figure.  
7. Order the heatmap of (7) by gene expression.  
8. Cluster the heatmap of (7) based on ChIP-seq signals of different marks.  
...
distance2NearestFeature(g.idh = [hip_peaks_in_GRanges], 
                        tss = [TSSs_in_GRanges])  
```
...
> **Exercise 7a**: Calculate the average H3K4me3 signal in a window of 5kb around the TSS.  
> 
...
> **Exercise 7b**: Transform the `average_h3k4me3_tss` to a data.frame (code check).  
>
> * Make sure to hit "Run Code" to view the output of the `head()` argument.  
> * Use this output to determine the name of the variable with the H3K4me3 scores.   
...
> **Exercise 7c**: Use the `left_join()` function to combine `average_h3k4me3_tss_df` and `quantification_chr19_sample` (code check). 
>
...
>**Exercise 8a**: Use the code from [Exercise 7a], [Exercise 7b], [Exercise 7c] and [Exercise 7d] to plot the average H3K7me3 signal in a 5kb window around the TSS for genes grouped in quantile of gene expression (code check).    
> 
...
grade_result(
   ...
   incorrect = "Copy paste and adjust the code of exercise 7d."
)
```
...
<br>
In heatmaps of genomics data, each row represents an interval, often aligned at the genomic feature in the middle. The signal strength at each bp or within each bin is encoded by a color scale (eg. white for backgroun signal, bright red for strong signal). 
<br>
...
* Use `col =` to define your own color palette. This palette should be provided as a vector.  
* `...` means that there are additional, optional arguments, some of which we will come back to soon.   


#### Defining your color palette  
There are many r packages out there that help you generate color palettes (eg.. RColorBrewer, viridis, rcartocolor and more). We suggest you work with the `colorRampPalette()` function for these heatmaps. `colorRampPalette()` takes a set of colors as input and returns a function that can subsequently be called with the integer argument `n=` and will return a palette with `n` entries.   
<br>
Here's an example of a red-to-blue palette with 4 colors applied to the boxplot of [Exercise 7d]:   

```{r create_colpal_example, eval=FALSE, echo=TRUE}
# Create function
...
```{r create_colpal_example2, eval=FALSE, echo=TRUE}
# Skip creating the palette function but immediately create your color palette
my_palette <- colorRampPalette(c('red', 'blue'))(4)
```
<br>  

> You will use `colorRampPalette()([n])` with `n=100` to generate white-to-[color]-colorscales that encode low/background-to-maximum signal measured in these ChIPs.      

### Exercise 8  

>**Exercise 8**: Generate a heatmap of the H3K4me3 ChIP-seq signal in a 10kb window around the TSS on a white-to-red color scale.  
> 
...
```{r q8_heatmap1mark, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.lines=18}
...
h3k4me3_cols <- colorRampPalette(c("white", "red"))(10000)
...
```{r q8_heatmap1mark-hint-1}
# Complete step 1 using the resize() function
```

```{r q8_heatmap1mark-hint-2}
# Review exercise 3 to complete step 1 and step 2
```

```{r q8_heatmap1mark-hint-3}
# Review the introduction to this section (3.5.2) to complete the heatMatrix() code   
```

```{r q8_heatmap1mark-hint-4}
# obtain 10kb windows around the TSS you should use
tss_window <- resize(tss_chr19_sample, width = 10001, fix = "center")
```

```{r q8_heatmap1mark-hint-5}
...
### 3.5.3 Heatmap of 1 mark ordered H3K4me3 signal  

### Exercise 9  

>**Exercise 9**: Repeat the heatmap of [Exercise 8] but this time ensure that it is *ordered by H3K4me3 signal* (code check).   

> * `scores_h3k4me3_tss` and `h3k4me3_cols` generate in exercise 8 are available to you.   
> * Make sure to hit "Run Code" to view the resulting plot.  

```{r prepare_q9a, echo=FALSE}
# obtain 10kb windows around the TSS you should use
tss_window <- resize(tss_chr19_sample, width = 10001, fix = "center")
# to generate the scorematrix, you should use
scores_h3k4me3_tss <- ScoreMatrix(target = bw_h3k4me3, 
...
```{r q9_heatmap1mark, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.lines=18, exercise.setup="prepare_q9a"}
...
Instead of ordering the heatmap by the plotted signal, we can also order it by the expression of the corresponding gene. To do so, 
```{r heatmap_fpkmorder, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.setup = "prepare_q5"}
# order TSS regions by gene expression from high-to-low
tss_chr19_sample_arranged  <- sort(tss_chr19_sample, by = ~FPKM, decreasing =TRUE)
tss_chr19_sample_arranged_window <- resize(tss_chr19_sample_arranged, width = 10001, fix = "center")
# generate scorematrix again
scores_h3k4me3_tss <- ScoreMatrix(target = bw_h3k4me3, windows = tss_chr19_sample_arranged_window, type = 'bigWig', strand.aware =TRUE) 
# plot heatmap
heatMatrix(scores_h3k4me3_tss, xcoord = c(-5000, 5000), col = colorRampPalette(c("white", "red"))(10))
heatMatrix(scores_h3k4me3_tss, xcoord = c(-5000, 5000), col = colorRampPalette(c("white", "red"))(100))



```

### 3.5.4 Same order, different marks   
available bigwigs = 
bw_h3k4me3
bw_h3k4me1
bw_h3k27ac
...
```{r example_heatmap_customorder, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.setup = "prepare_q5"}
# generate scorematrix again
scores_h3k4me3_tss <- ScoreMatrix(target = bw_h3k36me3, windows = tss_chr19_sample_arranged_window, type = 'bigWig', strand.aware =TRUE) 

# plot heatmap
heatMatrix(scores_h3k27ac_tss[my_order$gene_rank,], xcoord = c(-5000, 5000) , col = colorRampPalette(c("white", "red"))(100))
```

```{r}
#Which mark reflects the H3K4me3 pattern the most?   
```
...
```{r example_heatmap_ordered, eval = F}
list_3colors <- list(col = colorRampPalette(c("white", "green4"))(100), col = colorRampPalette(c("white", "red"))(10), col = colorRampPalette(c("white", "blue"))(100))
list_3colors_v2 <- list(col = colorRampPalette(c("white", "red"))(10), col = colorRampPalette(c("white", "red"))(10), col = colorRampPalette(c("white", "red"))(10))
...
# plot multiple marks in heatmap
multiHeatMatrix(scores_h3k4me3_h3k27ac_h3k27me3, xcoord = c(-5000, 5000), order = T, clustfun = function(x) kmeans(x, centers = 4)$cluster, col = list_3colors)

# scale and plot scaled --> can identify bivalent regions
scaled <- scaleScoreMatrixList(scores_h3k4me3_h3k27ac_h3k27me3)
multiHeatMatrix(scaled, xcoord = c(-5000, 5000), order = T, clustfun = function(x) kmeans(x, centers = 4)$cluster, col = list_3colors)
multiHeatMatrix(scaled, xcoord = c(-5000, 5000), order = T, clustfun = function(x) kmeans(x, centers = 4)$cluster, col = list_3colors_v2)
