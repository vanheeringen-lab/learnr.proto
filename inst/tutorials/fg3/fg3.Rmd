...
First, you will assign ChIP-seq peaks to the closest TSS. To do so you will again use functions from the GenomicRanges packages. There are two options:     

1. The `nearest(x, subject)` function detects for each interval in `x` the nearest interval within the set of regions reported in `subject`. This function outputs a vector of integers reporting for each region in `x` the index of the nearest neighbor range in `subject`.  
2. The `distanceToNearest(x, subject)` function does the same as but additionally reports the distance between intervals in `x` and the nearest neighbor in `subject`. This function outputs a `Hits` objects (similar to the output of `findOverlaps()`) with the absolute distances as a column in the metadata slot of the object.  
<br>
...
> 
- Ensure that the output reports the distances between the corresponding H3K4me3 peak and its nearest TSS. - Print the resultin gobjec tot the console using `show()`.  
...
# within distanceToNearest define x as:
x = monocytes_h3k4me3
```
...
<br>
You will thus plot the distribution of absolute distances between peaks and TSSs. "Absolute" in this context means that distances are reported irrespective of whether the corresponding TSS is located upstream or downstream the H3K4me3 peak.  
...
tss_near_h3k4me3 <- distance2NearestFeature(monocytes_h3k4me3, tss_chr19)
```
...
as.data.frame(mcols(tss_near_h3k4me3))
```

```{r q2a_metadata-check}
grade_result(
 pass_if(~identical(.result, head(metadata)))
 )
```

<!-- > **Exercise 2b:** Next, print a summary of the distances reported in `metadata`  -->
...
> **Exercise 2b:** Use ggplot() and geom_histogram() to plot these distances with default binwidth and default plot settings  

```{r prepare_q2c, echo = F}
tss_near_h3k4me3 <- distanceToNearest(monocytes_h3k4me3, tss_chr19)
metadata <- as.data.frame(mcols(tss_near_h3k4me3))
```

```{r q2c_plot, exercise = TRUE, exercise.eval = F, error = T, exercise.setup = "prepare_q2c"}
...
```{r q3_filter, exercise = TRUE, exercise.eval = F, error = T, exercise.setup = "prepare_q2c"}
# filter h3k4me3_to_tss for distances <= 10000
tss_near_h3k4me3 <- __[__<= 10000,]
```
