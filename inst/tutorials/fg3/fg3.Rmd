...
```
By running `ntile()` within `mutate()` you can define the new variable. For example, below we categorize genes into either quantiles and quintiles based on their gene expression in `quantification_chr19`:   
...
<br>
It can sometimes be hard to view trends with just points alone. Many times we wish to add a smoothing line in order to see what the trends look like. This can be added to the plot with `geom_smooth()`, which also adds a confidence interval of the smooth:   
```{r adding_smoothline, eval=FALSE, echo=TRUE}
ggplot(average_h3k4me3_tss_df, mapping=aes(y = H3K4me3_rpm, x = log(FPKM+1)))+  
  geom_point()+
  geom_smooth(fill = "blue")+ # we add a default smoothline
  theme_bw()+
  labs(title = "Average H3K4me3 signal at TSS versus gene expression in monocytes .",
...
You can use different methods to draw the trend line. For example, `methods = "lm" fits a linear model, gives the line of best fit:  
```{r adding_smoothlinefit, eval=FALSE, echo=TRUE}
ggplot(average_h3k4me3_tss_df, mapping=aes(y = H3K4me3_rpm, x = log(FPKM+1)))+  
  geom_point()+
  geom_smooth(fill = "blue")+ # we add a default smoothline
  theme_bw()+
...
# remember to define this as factor!
```
...
### Scatterplot of H3K27me3 signal vs gene expression   
...
### 3.5.3 Heatmap of ordered by H3K4me3 signal  
You can add `order=TRUE` to the `heatMatrix` function to order the rows in the heatmap by plotted value, in this case the H3K4me3 signal.  

### Exercise 10  

>**Exercise 10**: Repeat the heatmap of [Exercise 9] but this time ensure that it is ordered by H3K4me3 signal (code-check).   

> * `scores_h3k4me3_tss` and `h3k4me3_cols` generated in exercise 9 are available to you.  
...
heatMatrix(mat = scores_h3k4me3_tss, xcoord = c(-5000, 5000), 
           col = h3k4me3_cols, xlab = "bp (TSS = 0)",
...
# The next hint contains the solution!
```
