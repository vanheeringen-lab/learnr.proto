...
* renamed `V1` to `H3K4me3_rpm` using `rename()` from dplyr;  
...
```{r h3k4me3_boxplot_example, echo=TRUE, eval=F}
...
```{r h3k4me3_boxplot_example2, echo=FALSE}
...
ggplot(average_h3k4me3_tss_df) + 
  ...
  theme_bw()
```
To generate this boxplot for groups of genes grouped into four bins by gene expression level, we first add thes JH3K4me3 scores to the gene expression data.  
...
```{r ntile_example, ehco=TRUE, eval=FALSE}
ntile(x = [variable to rank] , n = [number of bins])
```
By running `ntile()` within `mutate()` you can define the new variable.   

<br>
For example, below we, categorize genes into either quantiles or quintiles based on their gene expression in `quantification_chr19`:   
...
> * The object `expresssion_and_h3k4me3` is available.  
...
ggplot(expresssion_and_h3k4me3, mapping=aes(x = ___, y = ___))+
   ___()+
  theme_bw()
```

```{r q7e_signalfpkm-solution}
ggplot(expresssion_and_h3k4me3, mapping=aes(x = H3K4me3_rpm, y = log(FPKM+1)))+  
  geom_point()+
...
in 15kb window?  
Use the code from exercises 7a-7d to plot the relationship between 

```{r}
# Resize the window in "tss_chr_sample" to 10kb
tss_chr19_sample_window <- resize(tss_chr19_sample, width = 15001, fix = "center")
...
# convert to df
average_h3k27me3_tss_df <- as.data.frame(as(average_h3k27me3_tss, "matrix")) 
...
# join
expresssion_and_h3k27me3 <- right_join(x = quantification_chr19_sample, y = average_h3k27me3_tss_df, by = "gene_id")

# plot quantiles
expresssion_and_h3k27me3 %>%  
  mutate(gene_group = factor(ntile(FPKM, n=4))) %>% 
  ggplot(.)+  
...
# plot scatter
expresssion_and_h3k27me3 %>%  
  ggplot(., mapping=aes(x = log(FPKM+1), y = h3k27me3_rpm))+  
  geom_point()+ 
  theme_bw()
...
## 3.5 Heatmaps  
In heatmaps we can visualize individual loci 
