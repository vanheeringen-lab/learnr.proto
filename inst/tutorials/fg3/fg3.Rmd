...
load(rdata)
rm(rdata)
tss_chr19_sample <- tss_chr19_sample[-(tss_chr19_sample$gene_id =="554235")] # this geneID is not present in quantification data
...
quantification_chr19_sample <- quantification_chr19_sample[-(quantification_chr19_sample$gene_id =="114335"), ] # this geneID is not present in TSSs
...
## 3.5 Heatmaps  
In fg2 (and on a per-locus basis in fg1) we observed that some histome marks occur more often than expected at the promoter and that, at a per-bp-level, some marks co-occur more than others. In the profile plots of [3.3 ChIP-seq signal strength] we summarized that different marks have different signals strengths and that this profile is, for some marks, directional suggesting a functional association with gene expression. Lastly, in the box- and scatter plots of [3.4 Signal vs gene expression] we observed that the relationship between the signal of a histone mark at the TSS and a gene's expression is different depending on the mark and is never a hard, 1-to-1 relationship.  
<br>
A method to display these 

### 3.5.1 Heatmap of 1 mark unordered  
Summarize H3K4me3 ChIP-seq signal at all TSSs in a heatmap.  
10kb window
data of [Exercise 5]  
```{r example_heatmap, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.setup = "prepare_q5"}
# plot heatmap
heatMatrix(scores_h3k4me3_tss, xcoord = c(-5000, 5000), col = colorRampPalette(c("white", "red"))(100))
```

### 3.5.2 Heatmap of 1 mark ordered by gene expression   

`rank()` takes a group of values and calculates the rank of each value within the group, e.g. rank(c(21, 22, 24, 23)) has output [1] 1 2 4 3

```{r example_heatmap_customorder, exercise =TRUE, exercise.eval=FALSE, error=TRUE, exercise.setup = "prepare_q5"}
# define order 
my_order <- inner_join(x=as.data.frame(mcols(tss_chr19_sample)), y=quantification_chr19_sample , by = "gene_id") %>%  
  mutate(gene_rank = rank(dplyr::desc(FPKM))) %>% 
  dplyr::select(gene_rank)
# plot heatmap
heatMatrix(scores_h3k4me3_tss[my_order$gene_rank,], xcoord = c(-5000, 5000) , col = colorRampPalette(c("white", "green4"))(100))
```

### 3.5.3 Same order, different mark  
