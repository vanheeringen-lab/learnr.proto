...
grade_result(
  ...
  correct = "Delightful! This object, 'dist_h3k4me3_to_tss' reports for each H3K4me3 peak a nearest TSS. Which variable reports the distance between the peak and its nearest TSS?"
)
```

**Important to note**: there are positive and negative distances reported in `dist_h3k4me3_to_tss$dist.to.feature`. Positive values indicate that the H3K4me3 peaks is located **downstream** the TSS, negative values refer to H3K4me3 peaks located **upstream** the TSS.  
...
>
> * With the range of distances given in exercise 2a, play around with the `binwidth =` parameter in `geom_histogram()`. 
...
question("Which of the following statments are correct? Tip: you can also use the output of summary() of exercise 2a." ,
         answer("Over 50% of the datapoints fall within 10kb up- or downstream the TSS", correct = T, message = "Indeed, over 50% of H3K4me3 peaks are within +/- 10kb of the TSS. But also notice the  outliers we are having: peaks are assigned to TSSs that are over 100kb away! These might be H3K4me1 peaks in enhancers that are detected to cross-reactivity of the H3K4me3 antibody. Alterantively, through chromosomal looping between enhancer and promoter, these regions might be co-immunoprecipitated with the H3K4me3 ChIP."),
...
>#### **Optional exercise*:  
<details><summary>Want to plot the distance to the nearest TSS for other marks?... :</summary>   
In the chunk of exercise 2b, add a line of code with the `distance2NearestFeature()` function. The peaks of the other histone marks in our dataset are stored in GRanges objects: `monocytes_h3k4me1`, `monocytes_h3k27ac`, `monocytes_h3k27me3`, `monocytes_h3k9me3` or `monocytes_h3k36me3`.  
</details>  

## 3.3 ChIP-seq signal strength     
Distance and overlap may tell you which gene and/or regulatory element the mark is most likely associated. As we, for example, observed in fg2: the presence and absence of H3K4me3 and H3K27me3 at a gene's promoter are associated with its level of gene expression. But there is more to it!  
<br>
Not only the presence or absence but the actual signal from ChIP-seq marks can be associated with the expression level of the gene in question. Such associations between ChIP-seq signal strength and gene expression - being it positive or negative associations - point to a functional relationship. Think of the ChIP-seq mark recruiting basal transcription factors thereby promoting transcription or - the other way around - the placing of marks by proteins recruited to the transcriptional machinery in elongation mode.  
<br>
In both cases you will observe a positive association between ChIP-seq signal strength and gene expression. In research, such observations (or the absence thereof) provide valuable insights in the function of a mark and can help you decide on the direction and design of follow-up experiments.   
<br>
...
### 3.3.1 Importing ChIP-seq signal in r   
...
ScoreMatrix(target = [path_to_BigWig_file], 
            windows = [GRanges_object_with_intervals_of_interest], 
            type = "bigWig", 
...
* `...` means that there are additional, optional arguments, some of which we will come back to later on in the tutorial.  
  ...
  *Modified from [GenomationManual](https://www.bioconductor.org/packages/release/bioc/vignettes/genomation/inst/doc/GenomationManual.html#1_Introduction)*
...
> * Hit "Run Code" to view the structure of the resulting object.   
...
You can plot the average H3K4me3 signal stored in this ScoreMatrix with the `plotMeta()` function. This function works as follows:   
...
Clearly, we could have restricted the plot tp the region +/- 3kb the TSS as the H3K4me3 signal remains near background level outside that region.   
...
quiz(caption="",
  question("Which function would you change and how, to make sure that the signal in the plot of Exercise 3b is limited to +/-3kb around the TSSs?",
     ...
     question("Is H3K4me3 signal equally strong before and after the TSS or is there a difference?",
              ...
              answer("H3K4me3 signal is higher downstream the TSS compared to upstream.", correct = T, message = "Indeed, H3K4me3 signal is on average higher downstream the TSS."),
              allow_retry = T),
...
grade_result(pass_if(~identical(.result,plotMeta(mat = scores_h3k27ac_tss, xcoord = c(-5000, 5000)))))
```
...
### 3.3.4 Multiple profiles in one plot  
We can observe differences and similarities between two ChIP-seq datasets with respect to the peak strength and peak shape better when we plot them in one plot. This can be achieved by generating a "ScoreMatrixList" object and providing that to the `plotMeta()` function:    
...
>
> * The objects `scores_h3k4me3_tss` and `scores_h3k27ac_tss` of Exercise 3  ([3.3.1 Importing ChIP-seq signal in r]) and Exercise 4 ([3.3.3 Add H3K27me3 signal]) are available to you.   
...
> * The object `tss_window` of Exercise 3a ([3.3.1 Importing ChIP-seq signal in r]) is available again.     
...
scores_h3k4me3_h3k27ac_h3k27me3 <- new("____", list(h3k4me3=__, h3k27ac=__, h3k27me3=__))
...
To generate this boxplot for groups of genes grouped into four bins by gene expression, you first need to add the gene expression data to your data.frame.  
...
> * Review previous exercises (eg Exercise 5, section [3.3.4 Multiple density profiles in one plot]) for how to resize your tss window and generate the ScoreMatrix object.  
