...
<br>
You will plot the distances as a histogram and as a density plot. 

>#### **Refresher**: histograms and density plots  
...
h3k4me3_to_tss_close <- h3k4me3_to_tss[h3k4me3_to_tss_metadata$distance <= 10000,]
```
...
Use `h3k4me3_to_tss_close` to subset peaks to those that are less than 10 kb away from the TSS. You can than use the function `subjectHits()` to get the genes associated to each peak.  
<br>
...
```{r q4_closegenes-hing-5, eval = F}
# this would be a strategy, check whether you had the same or something different. 'Submit Answer' will evaluate whether the object you generated is the same as ours.
interesting_TSSs <- unique(subjectHits(h3k4me3_to_tss_close))
