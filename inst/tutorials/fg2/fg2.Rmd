...
### 2.3.1 Peak counts
A common first step in genomics data analysis is to make look at some basic characteristics of our data by making exploratory plots.  
<br>

### Exercise 2a:    

> Determine the number of H3K4me3 and H3K4me1 peaks in monocytes. Peak data are stored in the R objects `monocytes_h3k4me3` and `monocytes_h3k4me1`.    
...
```{r q2b_widthh3k4me3-code-check, eval = F}
grade_code(
...
```{r q2b_widthh3k4me1-code-check, eval = F}
grade_code(
...
* Print a summary of this object and identify the variable (column in metadata) that you can use for counting the number of peaks per chip (this will be the statistic used in the barplot).   

```{r monocytesall, exercise = T}
# summary of object 'monocytes_all'
monocytes_all
```

<div id="monocytesall-hint">
**Hint:** Use `print()` or `show()`
</div>

```{r monocytesall-solution, echo = F}
# summary of object 'monocytes_all'
print(monocytes_all)
```
...
As GRanges objects are not compatible with ggplot2, you have to extract the metadata and write it in a data.frame  

> Complete the code below to extract the metadata from the object `monocytes_all`.  
> 
...
> * Complete the code below to plot the number of peaks per ChIP in a bar chart using ggplot2 plotting.  

```{r prepare-q_barplotpeakcount} 
monocytes_all <- unlist(monocytes_list)
monocytes_all$chip <- names(monocytes_all)
monocytes_metadata <- as.data.frame(mcols(monocytes_all))
```

```{r q_barplotpeakcount, exercise = T, fig.width=4}
...
> * Finish the code below to add the 'peak_size' variable, holding the peak widths, to `monocytes_metadata`. 

```{r prepare-q4_addpeaksize, echo = F}
monocytes_all <- unlist(monocytes_list)
monocytes_all$chip <- names(monocytes_all)
monocytes_metadata <- as.data.frame(mcols(monocytes_all))
```

```{r q4_addpeaksize, exercise = T}
# add column 'peak_size' to monocytes_metadata with the width of the peaks from monocytes_all 
monocytes_metadata

```

```{r q4_addpeaksize-hint-1}
width(monocytes_all)
```

```{r q4_addpeaksize-solution}
monocytes_metadata$peak_size <- width(monocytes_all)
```

```{r q4_addpeaksize-code-check, eval = F}
grade_code()
```

> * Now plot the peak size distribution as a boxplot.   

```{r prepare-q4_plotpeaksize}
monocytes_all <- unlist(monocytes_list)
monocytes_all$chip <- names(monocytes_all)
monocytes_metadata <- as.data.frame(mcols(monocytes_all))
monocytes_metadata$peak_size <- width(monocytes_all)
...
ggplot(data = monocytes_metadata)+
  geom_boxplot(mapping = aes(x = chip, y = peak_size))
```
...
> * This doesn't look very informative, lets log-transform the y-axis:  

```{r plotpeaksizelog, exercise = T}
# plot the peak_size per chip as boxplot, log-transform the y-axis
__(data = __)+
  __(mapping = __(x = __, y = __), fill = "gray", col = "black", bins = 100)+
  theme_bw()+
...
Different data types have different peak shapes. TF ChIP-seq data generally shows sharp, "narrow" peaks while ChIP-seq peaks of epigenomic data can be **narrow** or **broad**, depending on the mark or factor. Some data even show a **mixture** of narrow and broad peaks (e.g.ChIP-seq of RNA polymerase II) - **mixed** data are not part of this tutorial.  
<br>
Differences in peak shapes are accompanied by differences in enrichment scores: sharp, narrow peaks show higher enrichment and higher scores than broad domains.  
<br>
...
length(unique(___(___)))/length(___)
```
