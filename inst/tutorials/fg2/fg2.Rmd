...
> You will make a barplot showing the number of peaks per ChIP experiment.  

- GRanges objects of each mark are concatenated in the GRangesList object `monocytes_list`.  
- We applied the `length()` function on each object in this list.  
- Hit "Run Code" to repeat these steps and show the output

```{r peakcount, exercise = T}
# obtain number of peaks for each element in monocytes_list:
peak_counts <- sapply(monocytes_list, length)
# convert output to a data.frame 
peak_counts <- as.data.frame(peak_counts)
# add a column for the chip target 
peak_counts$chip <- rownames(peak_counts)
# print the result
peak_counts
```
> Plot the number of peaks per ChIP in a barplot using `ggplot()` and `geom_bar()`  
...
```{r barplotpeakcount-solution, exercise = T}
# plot the number of peaks per chip
...
>
> 1. Extract the metadata of H3K4me3 peaks and write it into a dataframe object 'metadata_h3k4me3'
> 2. Define 'peak_size' variable, holding the peak widths. 

```{r q4_extractmetadata, exercise = T}
# extract metadata 
...
```{r q4_extractmetadata-solution, eval = F}
# extract metadata 
...
```{r q4_extractmetadata-code-check, eval = F}
grade_code()
```

### Exercise 4b:   

> Now plot the peak size distribution of H3K4me3 peaks as a histogram using `ggplot()` and `geom_histogram()`  

```{r prepare-q4plotpeaksizeh3k4me3}
# extract metadata 
metadata_h3k4me3 <- as.data.frame(mcols(monocytes_h3k4me3))
# add column 'peak_size' with the width of H3K4me3 peaks
metadata_h3k4me3$peak_size <- width(monocytes_h3k4me3)
...
```{r q4plotpeaksizeh3k4me3, exercise = T}
# plot the H3K4me3 peak_size as a histogram:  
..(data = metadata_h3k4me3)+
  ..(mapping = ..(x = ..), fill = "gray", col = "black", bins = 100)+
  theme_bw()
```

<div id="q4plotpeaksizeh3k4me3-hint">  

- use ggplot() and geom_histogram()   
- Map peak_size to the x-axis.    
- `mapping =` is always paired with `aes()`.    

</div>

```{r q4b_plot_peaksize_h3k4me3-solution, eval = F}
# plot the H3K4me3 peak_size as a histogram:  
...
```{r }
# plot the H3K4me3 peak_size as a boxplot:  
ggplot(data = metadata_h3k4me3)+
  geom_histogram(mapping = aes(x = peak_size), fill = "gray", col = "black", bins = 100)+
  theme_bw()
```
