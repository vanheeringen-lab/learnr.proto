...
As you can see, the specific combination of histone marks at the promoter can be functionally associated with a different output in gene expression levels. Let's not restrict our analysis to 2 marks but examine the occurrence at the promoter of all the different marks in our dataset.  

## 2.6 Quantify and visualize overlap among > 4 marks   
### Upset plots  
An [UpSet plot](https://jku-vds-lab.at/tools/upset/) is an elegant alternative to the VennDiagram when you want to visualize the intersection among more than 3 sets (also see this [commentary in Nature Methods 2014 by Alexander Lex & Nils Gehlenborg](https://www.nature.com/articles/nmeth.3033)). It combines  

- UpSet plots the intersections of a set as a matrix, with barplots that visualize (a) the size of the individual sets, and (b) the size of the detected intersections.  
- Additional attributes of the intersections can be added to the bottom or sides of the plots, which we will do as well.  
...
```{r upset_marks_at_promoters}
overlap_all <- lapply(monocytes_list, function(x) promoters_chr19[unique(subjectHits(findOverlaps(query = x , subject = promoters_chr19)))]$gene_id)
names(overlap_all) <- names(monocytes_list)
m1 = make_comb_mat(overlap_all, universal_set =  promoters_chr19$gene_id)
UpSet(m1,
      comb_order = order(comb_size(m1), decreasing =  TRUE),
      set_order = order(set_size(m1), decreasing = FALSE),
      #comb_col = brewer.pal("Set1", n=7)[comb_degree(m1)]
      )

## use plot to set threshold:
barplot(sort(comb_size(m1)))
abline(h = 40, lwd =2, col = "red")
abline(h = 50, lwd =2, col = "black")
abline(h = 60, lwd =2, col = "blue")

## upset of filtered matrix
m2 <- m1[comb_size(m1) >40]
UpSet(m2,
      comb_order = order(comb_size(m2), decreasing =  TRUE),
      set_order = order(set_size(m2), decreasing = FALSE),
      )

m3 <- m1[comb_size(m1) >60]
UpSet(m3,
      comb_order = order(comb_size(m3), decreasing =  TRUE),
      set_order = order(set_size(m3), decreasing = FALSE),
      )

## add average gene expression as boxplot below the combinations
comb_sets <- lapply(comb_name(m3), function(nm) extract_comb(m3, nm))
...
**So far we have only looked at individual overlaps but chromatin states are actually defined by the combinatorial occupancy of different marks and factors. Let's analyze the chromosome19-wide overlap of the marks in our dataset and identify the occurring combinations.**  

## 2.6 Summarizing overlap among marks   
### Upset plots  
Lastly, you will interpret UpSet plots generated using the `ComplexHeatmap` package. The UpSet plot is an elegant alternative to the VennDiagram when working with more than 4 datasets (see this [commentary in Nature Methods 2014 by Alexander Lex & Nils Gehlenborg](https://www.nature.com/articles/nmeth.3033)).   
<br>
An UpSet plot visualizes the intersections of multiple sets, sets are the ChIP-seq peaks in our case. The size of the sets are total number of base pairs covered by that mark. The size of the combination sets refers to the sum of base pairs covered by the respective overlapping regions. So, if a peak only partially overlaps, only the base pairs that are part of the overlap will be counted.   

<!-- >#### **Background info**: 3 possible ways for defining combination sets.    -->
<!-- <details><summary> Click here for more info on the 3 modes defined by the `ComplexHeatmap` package.</summary> -->
...
<!-- >  -->
<!-- > ![](images/wk2_4_complexheatmap_modes.png){width=400}\        -->
...
To detect combinations you use the `make_comb_mat()` function:  
```{r makecombexample, eval=FALSE, echo=TRUE}
combination_matrix <- make_comb_mat([list_of_objects_to_be_evaluated]) 
```

* The "[list_of_objects_to_be_evaluated]" should be a named list. Meaning that each element in the list should have a name attached to it.   

<!-- * The returned object is a "combination matrix" object.   -->
<!-- * There are special functions to view the content of this object:   -->
<!--     - `set_name()` prints the names of the objects in the "[list_of_objects_to_be_evaluated]".     -->
...
<!--         + The combinations are encoded in a binary fashion with 0s and 1s.   -->
<!--         + A 1 means the mark in question is present in the combination.   -->
<!--         + These 0s and 1s follow the order given by `set_name()`.    -->
* You can visualize the detected overlap with the `UpSet()` function:  

```{r upsetexample, eval=FALSE, echo=TRUE}
UpSet(m = [combination_matrix], .....)
```

* `m` = the output of the `make_comb_mat()` function.  
* `...` means that there are more, optional arguments some of which we will use later on.  

### Exericse 13   

In the following example we run the two functions to make and plot the combination matrix of `monocytes_h3k4me3` and `monocytes_h3k27me3`:    
<br>
```{r makecombexample_realdata, eval=FALSE, echo=TRUE}
## 1. make list of the two objects and name the elements
my_list <- GRangesList(monocytes_h3k4me3, monocytes_h3k27me3)
names(my_list) <- c("h3k4me3", "h3k27me3")

## 2. calculate the combination matrix
comb_matrix <- make_comb_mat(my_list)

## 3. plot the UpSet plot
# a. order the combinations from least to most frequent
# b. adjust top and right axis labels to make them better readable.
UpSet(m = comb_matrix, 
      comb_order = order(comb_size(comb_matrix)),
      top_annotation = upset_top_annotation(  
        comb_matrix,
        axis_param = list(at = c(0, 2.5e6, 5e6),
                          labels = c("0Mb", "2.5Mb",  "5Mb")),
        height = unit(3, "cm")),
      right_annotation = upset_right_annotation(
        comb_matrix,
        axis_param = list(at = c(0,2e6,4e6,6e6),
                          labels = c("0Mb", "2Mb", "4Mb", "6Mb"),
                          labels_rot = 0),
        width = unit(3, "cm")))
```
<br>
![](images/wk2_5_upsetplots_q13.png)\  
<br>
In this UpSet plot:  

- The total bps covered by the original objects is plotted in the barplot to the right.  
- The size of the intersection (=overlap) is plotted by the barplot on top of the plot.   
- The bars for overlap and no-overlap are annotated by dots and a connector line. When a line connects several dots it means that the bar above it reflects the combination of the connected datasets.  


```{r q13_upsetplot_mc}
quiz(caption ="", 
     question("Which of the two mark covers most bps?",
              answer("H3K4me3", message = "Use the barplot plotted to the right of the figure."),
              answer("H3K27me3", correct=TRUE, message = "We can see this in the barplot to the right (labelled with set size)"),
              answer("Both cover equal number of bps", message = "Use the barplot plotted to the right of the figure."),
              allow_retry=TRUE),
     question("Based on this graph, do you find the combination of H3K4me3 and H3K27me3 more often compared to each mark singularly?",
              answer("The combination of H3K27me3 & H3K4me3 is found more often than the marks singularly"),
              answer("H3K27me3 and H3K4me3 are found more often seperate from eachother than together.", correct = TRUE),
              allow_retry=TRUE)
     )
```
...
### Exericse 14      
We also generated an UpSet plot of all marks, restricted to the combinations of at least 100kb and left out regions that are covered by one mark only:  
<br>
```{r makefig14, eval=FALSE, echo=TRUE}
## 1. Calculate the combination matrix
monocytes_combinationmatrix <- make_comb_mat(monocytes_list)

## 2. Filter combination matrix for combinations of at least 100kb
monocytes_combmatrix_filt <- monocytes_combinationmatrix[comb_size(monocytes_combinationmatrix) > 100000]

## 3. Remove the 'combinations' larger than 3kb
# these are windows covered by one mark
monocytes_combmatrix_filt <- monocytes_combmatrix_filt[comb_size(monocytes_combmatrix_filt) < 3000000]

## 4. plot the UpSet plot
# a. order the combinations from least to most frequent
# b. adjust top and right axis labels to make them better readable.
UpSet(monocytes_combmatrix_filt,
      set_order = order(set_size(monocytes_combmatrix_filt), decreasing = F),
      comb_order = order(comb_size(monocytes_combmatrix_filt), decreasing = T),
      top_annotation = upset_top_annotation(
        monocytes_combmatrix_filt,
        axis_param = list(at = c(0, 0.5e6,1e6,1.5e6, 2e6),
                          labels = c("0Mb", "0.5Mb", "1Mb", "1.5Mb", " 2Mb")),
        height = unit(4, "cm")),
      right_annotation = upset_right_annotation(
        monocytes_combmatrix_filt,
        axis_param = list(at = c(0,4e6,8e6,12e6),
                          labels = c("0Mb", "4Mb", "8Mb", "12Mb"),
                          labels_rot = 0),
        width = unit(3, "cm")))
```
<br>
![](images/wk2_5_upsetplots_q14.png)\  
<br>

```{r q14_upsetplot_mc}
quiz(caption ="", 
     question("Which mark covers most bps when taking all marks into consideration?",
              answer("H3K4me3"),
              answer("H3K4me1"),
              answer("H3K36me3"),
              answer("H3K27ac"),
