...
```{r q6d_overlap_enterfraction, exercise = T, exercise.setup = "prepare_exercise6d"}
...
```{r q6e_overlap_enterfraction, exercise = T, exercise.setup =  "prepare_exercise6e"}
# enter the fraction of promoters that are part of the overlap, round to 3 decimal places:  
...
<br>
```{r q7b_binomtest, exercise = T, exercise.eval = F, error = T}
...
```{r q8a_genesids_overlap, exercise = T, exercise.eval = F, error = T, exercise.setup = "prepare_exercise8"}
...
```{r q9a_geneexpression, exercise = T, exercise.eval = F, error = T, exercise.setup = "prepare_exercise9a"}
...
# Define a new variable that identifies if a gene is present in the overlap or not.  
quantification_chr19_new <- quantification_chr19 %>% 
  mutate(h3k4me3_promoter = entrez_id %in% promoters_with_h3k4me3$gene_id )
```
...
```{r q9b_plot_genequant, exercise =  T , exercise.eval = F, error = T, exercise.setup = "prepare_exercise9b"}
...
```{r q9c_plot_genequant, exercise =  T , exercise.eval = F, error = T, exercise.setup = "prepare_exercise9b"}
...
# 3. define a "h3k27me3_promoter" in `quantification_chr19`, pipe the output in your ggplot command  
quantification_chr19 %>%  
  mutate(h3k27me3_promoter = entrez_id %in% promoters_with_h3k27me3$gene_id) %>% 
  ggplot(.) +
...
quantification_chr19_new <- quantification_chr19 %>% mutate(
  promoter_mark = case_when(
    entrez_id %in% promoters_with_h3k4me3$gene_id & entrez_id %in% promoters_with_h3k27me3$gene_id ~ "____",  # promoters with both marks
    entrez_id %in% promoters_with_h3k4me3$gene_id & !(entrez_id %in% promoters_with_h3k27me3$gene_id) ~ "____", # promoters with only h3k4me3
    !(entrez_id %in% promoters_with_h3k4me3$gene_id) & entrez_id %in% promoters_with_h3k27me3$gene_id ~ "____", # and so forth... 
    !(entrez_id %in% promoters_with_h3k4me3$gene_id | entrez_id %in% promoters_with_h3k27me3$gene_id) ~ "____") ) 
...
`quantification_chr19 %>%  mutate(promoter_mark = case_when(`
    `entrez_id %in% promoters_with_h3k4me3$gene_id & entrez_id %in% promoters_with_h3k27me3$gene_id ~ "bivalent",` 
    `entrez_id %in% promoters_with_h3k4me3$gene_id & !(entrez_id %in% promoters_with_h3k27me3$gene_id) ~ "H3K4me3_only", `
    `!(entrez_id %in% promoters_with_h3k4me3$gene_id) & entrez_id %in% promoters_with_h3k27me3$gene_id ~ "H3K27me3_only",` 
    `!(entrez_id %in% promoters_with_h3k4me3$gene_id | entrez_id %in% promoters_with_h3k27me3$gene_id) ~ "no_mark") )`
</div>
...
<br>
The `make_comb_mat([list_of_grangesobjects])` is used to find overlapping marks. It returns a "combination matrix" object. Special functions can be used to view it's information and filter the reported combinations.   
<br>

### Exericse 12a    

> **Exercise 12a**: Run the code below, which calculates the combination matrix of the different ChIP-seq peak sets.   
> 
...
```{r q12a_upsetrplot_mark_overlap, exercise = T}
...
comb_size(monocytes_combinationmatrix)
```
...
```{r q12b_upsetrplot, exercise = T, exercise.setup = "prepare_exercise12b", exercise.eval = F, error = T}
...
```{r q12c_niceupset, exercise = T, exercise.setup = "prepare_exercise12b"}
# filter monocytes_combinationmatrix for combinations covering at least 200kb
monocytes_combmatrix_filt <- monocytes_combinationmatrix[comb_size(monocytes_combinationmatrix) > 100000]
...
```{r add_distance_to_tss, echo = T, eval = F}
# filter monocytes_combinationmatrix for combinations covering at least 200kb
monocytes_combmatrix_filt <- monocytes_combinationmatrix[comb_size(monocytes_combinationmatrix) > 100000]
