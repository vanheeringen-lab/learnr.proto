...
> **Exercise 6a:** Use `findOverlaps()` to determine the overlap between (query) `monocytes_h3k4me3` and (subject) `promoters`.   
> 
> - you need to hit "Run Code" to be able to see the output of `show()`   


```{r exericse6, exercise = T}
...
```{r exericse6-code-check, eval = F}
grade_code()
```
<br>
The Hits object "overlap" reports the indices of `monocytes_h3k4me3` and `promoters` that overlap. If a peak or a promoter overlaps several times, each overlap will be reported on a new row.  
<br>

### Exercise 6b and 6c  

> **Exercise 6b:** What fraction of promoters is part of the overlap?  
>  
...
> **Exercise 6b:** What fraction of promoters that are part of the overlap?  

```{r q6b_overlap, exercise = T, exercise.setup = "prepare_exercise6b"}
...
```{r q6b_overlap-solution, eval = F}
# Extract the unique promoter indices
unique_promoters <- unique(subjectHits(overlap))

# How many of these unique promoters do you have?
unique_promoter_count <- length(unique_promoters)

# Print the resulting number
unique_promoter_count

# What fraction of promoters is part of the overlap?  
unique_promoter_count/length(promoters)
```

```{r q6b_overlap-code-check, eval = F}
# grade_result(
...
> **Exercise 6c:** What fraction of the h3k4me3 peaks are part of the overlap?  

```{r q6c_overlap, exercise = T, exercise.setup = "prepare_exercise6b"}
...
unique_peaks_count/__(monocytes_h3k4me3)
```
...
<br>
**Let's plot this overlap in a venn diagram.**   
...
```{r venndiagram, exercise = T, warnings =  F, message=F, exercise.setup = "prepare_exercise6b"}
...
```
<br>
*"I thought H3K4me3 was a promoter mark?! Why do we have so many H3K4me3 peaks that do not overlap with a promoter?!"*  


### 2.4.3 Overlap - testing for significance   
