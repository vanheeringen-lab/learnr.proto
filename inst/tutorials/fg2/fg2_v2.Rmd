...
> #### **Background info**: GRanges and rtracklayer are Bioconductor packages, but what is Bioconductor?   
...
> **Exercise 1:**  
Adjust the code to print the head of `monocytes_h3k4me3`.  
...
quiz(caption = "", 
    question("What are the median peak sizes for H3K4me3 and H3K4me1 ChIP-seq peaks in the current dataset? (in monocytes, restricted to chromosome19)",
             answer("average H3K4me3 peak = 879.5 bp", correct = T),
             answer("average H3K4me3 peak = 1662 bp"),
             answer("average H3K4me3 peak = 1198.6 bp"),   
             answer("average H3K4me3 peak = 2642 bp"),
             answer("average H3K4me1 peak = 879.5 bp"),
             answer("average H3K4me1 peak = 1662 bp", correct = T),
             answer("average H3K4me1 peak = 1198.6 bp"),   
             answer("average H3K4me1 peak = 2642 bp"), 
             allow_retry = TRUE),
...
```
<br>
The GRanges object structure is not compatible with ggplot2-ting. To plot these peak sizes using ggplot2, you therefore first have to write the metadata into a data frame using the `as.data.frame()` and `mcols()` functions.  
...
ggplot2 implements the grammar of graphics, an easy to use system to build up and customize your graphics [ref](https://doi.org/10.1198/jcgs.2009.07098).  
<br>
...
> <br>
**Scales** can be customized with `scale_[x/y]_[]` functions. These can be used to for example, change a color palette, set limits to the values included inthe mapping, or log transform an axis.  
<br>
...
```{r q3b_plot_peaksize_h3k4me3, exercise = T}
# plot the H3K4me3 peak_size as a histogram:  
ggplot(data = ...)+
  geom_histogram(mapping = ..(x = ..), fill = "gray", col = "black", bins = 100)+
  theme_bw()
```
...
As mentioned above, you can use dplyr-related functions from the plyranges package to wrangle, filter and manipulate GRanges objects.  
