...
## 1.2 From sample to raw data  {data-progressive=TRUE}  
...
>    iii) Ligate **adapters** to the fragment these contain a sample specific sequence (= **barcod** or **index**), regions complementary to oligo's that are found on the flow cell (these oligo's are called P5 and P7) and primer bindings sites for sequencing (Rd1 SP and Rd2 SP):  
...
<br>
![](images/wk1_2_fastq_example3.png)   
<br>
...
<br> 
The Phred scores represents the probability (p) that the reported call is incorrect:  
...
Single letters and symbols are used to represent each quality score, eg:  
...
## 1.3 From raw data to science  {data-progressive=TRUE}  
...
We run the raw fastq files through [FASTQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) software to get a quick impression of whether our data has any technical problems of which we should be aware before doing any further analysis. This tools outputs a HTML report with  graphs and tables summarizing, among others the base call accuracy, library base composition, duplication levels and the presence of overrepresented sequences.    
 ...
 he **Per base sequence quality** plot is one of the most important plots generated. It summarizes the base call accuracy (Y-axis) for each cycle (X-axis) across the library. See this example:  
 <br>
![](images/wk1_3_fastqc_per_base_quality_vb1.png){width=80%}  
For most runs, quality should be good for most reads through the whole run. If quality deteriorates we can use the other plots to understand why this is.  

- Good (Illumina) quality is generally Phred > 28 - 30    
- Concerning (Illumina) quality is Phred < 20  

> **Exercise 1**  
> Use the following two FASTQC reports to anser the following questions.  
> 
...
```{r quiz_fastqc, exercise = T, echo =  F}
quiz(caption = "Use the following two FASTQC reports to anser the following questions?",
...
1. Read name similar to name in fastq file line 1   
2. Flag, encodes alignment information of the read in 12 "bits". It can, for example, tell you whether the corresponding read is mapped, unmapped or a PCR or optical duplicate. This encoding is not the easiest to immediatetely understand, luckily there are webtools to summarize for us what each flag number means: [from samformat](https://www.samformat.info/sam-format-flag), or [Broad Institute](https://broadinstitute.github.io/picard/explain-flags.html).  
3. Name of the contig or chromosome in the reference genome the read aligns to: eg. `chr7` for chromosome 7 or `mt` for mitochondrial DNA    
5. Position within the chromosome the read aligns to: eg. `229418420`   
5. Mapping quality score, the program adds this score but different programs use different scores, can have value of `0` to `255`    
6. CIGAR string, gives a summary of the alignment per nt: eg. `51M`, encodes information about among others the number of matching nt's, whether there is a deletion  
7. Name of the reference genome where the paired read aligns to: eg. `chr7` (if there is no paried read `0`)   
8. Position within the reference genome where the paried read aligns to: eg. `229418621` 
9. The length of the complete fragment, from the first mapped base of read 1 to the last mapped base of read 2.  
10. Read sequence, similar to line 2 in fastq file.   
11. Phred quality score of the read (ASCII encoded), similar to line 4 of the fastq file, eg: `CCCFFFHHJJJAA...`  
...
C.  The chemical reaction and quality of the optical measurement that occur in the sequencer are not perfect and depend among others on the nucleotide composition of all samples, the length of the DNA molecules and the concentration of the samples. Each nt is therefore given a quality score that reflects the accuracy of the measurement.   
...
b. Remove low-quality nt's from reads or remove reads altoghether if all their nt's are of low quality.  
c. Filter these out of our dataset right after we get our data from the sequencer.  

```{r quiz_artefacts, echo =  F}
...
### <span style="color: red;">1.3.3 Output files: bigwigs, peaks & counts</span>    
...
1. UCSC Genome Browser Basics (duration (mm.ss): 08.40): ![](https://www.youtube.com/watch?v=DNXI-M9oQl8)    
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Ensure that "Human" is selected in the **REPRESENTED SPECIES** tree and "Dec. 2013 (GRCh38/hg38)" is selected under **Human Assembly**.</span>     
<br>
...
![](images/wk1_4b_ucsc_gb_actb_question_startpage.png){width=80%}\    
...
![](images/wk1_4b_ucsc_gb_actb_locationinchromosome.png){width=80%}\     
<br>
```{r q1_actin_location, eval = T, echo = F}
...
![](images/wk1_4b_ucsc_gb_unselectsplicevariants.png){width=80%}\       
<br>
Your GB session should look like this: ![](images/wk1_4b_ucsc_gb_actb_genetracksonly.png){width=80%}\      
<br>
...
![](images/wk1_4b_ucsc_gb_actb_question_TSSquestion.png){width=80%}\   
```{r 13_ACTB_start, echo = F}
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Upload the required data using the following URL [...].</span>     
<br>  
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Go back to the *ACTB* locus and zoom out 3x to see the surrounding region.</span>     
...
```{r Q8_ACTB_histone_marks_one, echo = F}
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Upload the peak data.</span>     
<br>
...
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Zoom into that gene *uncluding* the prominant H3K27me3 peak.</span>     
...
<br>
```{r Q9_FSCN1_bivalent_promoter, echo = F}
...
```
<br>
```{r Q12_ACTB_enhancer_two, echo = F}
...
```{r Q15_cd14_near_silencing, echo = F}
...
```
<br>
```{r Q16_cd14_near_silencing_two, echo = F}
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: Go to the ACTA1 gene, encoding muscular actin. Zoom out 10x.</span>  

> *Hint*: you should be at ??  chr1:229,418,420-229,446,919.  

<br>
```{r Q17_ACTA1_marking_one, echo = F}
...
<br>
```{r Q18_ACTA1_marking_two, echo = F}
...
<br>
<span style="color: goldenrod; font-family: Arial; font-size:16px;">ACTION: upload the following track hub [URL].</span>   
<br>  
...
```{r Q19_recognizing_patterns, echo = F}
question("Question 19: What is the most likely encoding?",
  answer("encoding 1"),
  answer("encoding 2", correct = T),
  answer("encoding 3"),
  answer("encoding 4"),
  allow_retry = TRUE
)
